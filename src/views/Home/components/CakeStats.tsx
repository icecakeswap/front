...
const CakeStats = () => {
  ...
  const snakeswapPrice = usePriceCakeBusd()
  const circSupply = totalSupply ? totalSupply.minus(burnedBalance) : new BigNumber(0)
  const cakeSupply = getBalanceNumber(circSupply)
  const marketCap = snakeswapPrice.times(circSupply)

  let snakeswapPerBlock = 0
  if (farms && farms[0] && farms[0].snakeswapPerBlock) {
    snakeswapPerBlock = new BigNumber(farms[0].snakeswapPerBlock).div(new BigNumber(10).pow(18)).toNumber()
  }
